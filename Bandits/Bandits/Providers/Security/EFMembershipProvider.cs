...
namespace Bandits.Providers.Security
{
    public class EFMembershipProvider : MembershipProvider
    {
        ...
        public override string GetPassword(string email, string answer)
        {
            ...
            using (WebUsersController wuc = new WebUsersController())
            {
                WebUser user = wuc.GetWhere(swu => swu.Email.Equals(email)).FirstOrDefault();
        ...
        public override bool ChangePassword(string email, string oldPassword, string newPassword)
        {
            ...
            {
                WebUser user = swuc.GetWhere(u => u.Email == email).FirstOrDefault();
        ...
        public override void UpdateUser(MembershipUser membershipUser)
        {
            using (WebUsersController swuc = new WebUsersController())
            {
                WebUser user = swuc.GetWhere(u => u.Email == membershipUser.Email).FirstOrDefault();
        ...
        {
            Expression<Func<WebUser, bool>> search = user => (user.WebUserId == (Int32)providerUserKey);
            return GetUser(search, userIsOnline);
        }

        public override MembershipUser GetUser(string email, bool userIsOnline)
        {
            Expression<Func<WebUser, bool>> search = user => (user.Email == email);
            return GetUser(search, userIsOnline);
        }
        ...
        public override string GetUserNameByEmail(string email)
        {
            using (WebUsersController swuc = new WebUsersController())
            {
                WebUser user = swuc.GetBy(u => u.Email == email);
        ...
        private MembershipUser GetUser(Expression<Func<WebUser, bool>> search, bool userIsOnline)
        {
            using (WebUsersController swuc = new WebUsersController())
            {
        ...
        private WebUser GetUser(Expression<Func<WebUser, bool>> query)
        {
            WebUser user;
